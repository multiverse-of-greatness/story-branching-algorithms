...
def validate_story_data(config: GenerationConfig, story_data: StoryData):
    if len(story_data.main_scenes) < config.num_main_scenes:
        raise ValueError(f"Main scenes generated by model ({len(story_data.main_scenes)}) not equal to setting scenes ({config.num_main_scenes})")
    
    if len(story_data.main_characters) < config.num_main_characters:
        raise ValueError(f"Main characters generated by model ({len(story_data.main_characters)}) not equal to setting characters ({config.num_main_characters})")
    
    if len(story_data.endings) < config.num_endings:
        raise ValueError(f"Endings generated by model ({len(story_data.endings)}) not equal to setting endings ({config.num_endings})")
    
    if len(story_data.chapter_synopses) < config.num_chapters:
        raise ValueError(f"Chapter synopses generated by model ({len(story_data.chapter_synopses)}) not equal to setting chapters ({config.num_chapters})")
